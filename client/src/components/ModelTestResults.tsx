...
import { Shield, AlertTriangle, CheckCircle, XCircle, ChevronDown, ChevronRight, BarChart3, Clock, Zap, Download, Wrench, Loader2, MessageSquare } from 'lucide-react';
import SocialAwarenessPanel from './SocialAwarenessPanel';
...
export const ModelTestResults: React.FC<ModelTestResultsProps> = ({ result, sessionId, onPurify, onApprove, onReject, onRepair, repairStatus, repairLoading, repairError }) => {
  ...
  const [mainTab, setMainTab] = useState<'ethics' | 'social'>('ethics');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16, color: '#e0e0e0' }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: '#12121f', borderRadius: 8, padding: 4 }}>
        <button
          onClick={() => setMainTab('ethics')}
          style={{
            flex: 1, border: 'none', borderRadius: 6, padding: '8px 12px',
            background: mainTab === 'ethics' ? '#6366f1' : 'transparent',
            color: mainTab === 'ethics' ? '#fff' : '#888', fontWeight: 600, fontSize: 13,
            cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          }}
        >
          <Shield size={14} /> ETHOS Results
        </button>
        <button
          onClick={() => setMainTab('social')}
          style={{
            flex: 1, border: 'none', borderRadius: 6, padding: '8px 12px',
            background: mainTab === 'social' ? '#22c55e' : 'transparent',
            color: mainTab === 'social' ? '#fff' : '#888', fontWeight: 600, fontSize: 13,
            cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6,
          }}
        >
      ...
      {mainTab === 'social' && <SocialAwarenessPanel />}
      ...
      {mainTab === 'ethics' && <>
              ...
              <div>
                ...
                {repairStatus.result.round_history && repairStatus.result.round_history.length > 0 && (
                  <div style={{ background: '#1a1a2e', borderRadius: 6, overflow: 'hidden' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr 90px 100px', fontSize: 11, color: '#666', padding: '6px 10px', borderBottom: '1px solid #2a2a3e' }}>
                      <span>Round</span><span>Passed</span><span>Failed</span><span>Pass Rate</span><span>Verdict</span><span>Report</span>
                    </div>
                    {repairStatus.result.round_history.map((row: any, idx: number) => {
                      ...
                      return (
                        <div key={idx} style={{
                          display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr 90px 100px',
                          ...
                          {sessionId && (
                            <button
                              onClick={() => downloadPdf(
                                `${BASE}/api/model/${sessionId}/report/round/${row.round}`,
                                `round${row.round}_report_${sessionId.slice(0, 8)}.pdf`
                              )}
                              style={{ fontSize: 10, padding: '2px 6px', background: '#6366f1', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
                            >
                              Download
                            </button>
                          )}
                ...
                {sessionId && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                    <button
                      onClick={() => downloadPdf(
                        `${BASE}/api/model/${sessionId}/report/baseline`,
                        `baseline_report_${sessionId.slice(0, 8)}.pdf`
                      )}
                      style={{ ...btnStyle('#64748b'), display: 'flex', alignItems: 'center', gap: 5, fontSize: 12 }}
                    >
                      <Download size={12} /> Baseline Report
                    </button>
                    <button
                      onClick={() => downloadPdf(
                        `${BASE}/api/model/${sessionId}/report/final-verification`,
                        `final_verification_${sessionId.slice(0, 8)}.pdf`
                      )}
                      style={{ ...btnStyle('#22c55e'), display: 'flex', alignItems: 'center', gap: 5, fontSize: 12 }}
                    >
                      <Download size={12} /> Final Verification Report
                    </button>
                    <button
                      onClick={() => downloadPdf(
                        `${BASE}/api/model/${sessionId}/download-trained-model`,
                        `ethos_trained_model_${sessionId.slice(0, 8)}.zip`
                      )}
                      style={{
                        ...btnStyle('#6366f1'),
                    ...
                    >
                      <Download size={12} /> Download Trained Model (.zip)
                    </button>
                  </div>
                )}
              </div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
          {sessionId && (
            <button
              onClick={() => downloadPdf(
                `${BASE}/api/model/${sessionId}/report`,
                `ethos_report_${sessionId.slice(0, 8)}.pdf`
              )}
